...
### 3-1. 읽기 전용이 아닌 일반 에이전트 타입을 사용하라

QA 에이전트를 읽기 전용으로 설정하면 검증 스크립트 실행이 불가하다. 효과적인 QA는:
- Grep으로 패턴 검색 (모든 `NextResponse.json()` 추출)
- 스크립트 실행으로 자동 대조 (API shape vs 훅 타입)
- 필요 시 수정까지 가능

**권장**: `.gemini/agents/qa-inspector.md`에 일반 에이전트로 정의하되, 에이전트 정의에서 "검증 → 리포트 → 수정 요청" 프로토콜을 명시.
...
- 발견된 이슈를 `_workspace/qa_report.md`에 구체적으로 기록 (파일:라인 + 수정 방법)
- 경계면 이슈는 양쪽 에이전트가 담당하는 영역을 모두 명시
- 오케스트레이터에게: 검증 리포트 반환 (통과/실패/미검증 항목 구분)
```
