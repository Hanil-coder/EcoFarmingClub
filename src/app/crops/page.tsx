      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            ...
            <p className="text-stone-600 font-medium">도시에서 키우기 좋은 다양한 작물들의 재배 노하우를 확인하세요.</p>
          </div>
        ...
        {crops.length > 0 ? (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 sm:gap-8">
            {crops.map((crop) => (
              <div key={crop.id} className="group overflow-hidden rounded-[32px] bg-white shadow-sm transition-all hover:shadow-xl hover:-translate-y-1 border border-stone-100">
                ...
                <div className="p-6">
                  ...
                  <Link href={`/crops/${crop.id}`} className="block w-full text-center py-2.5 rounded-xl bg-stone-50 text-stone-700 text-xs font-bold transition-all hover:bg-brand-green hover:text-white shadow-sm border border-stone-100">
                    가이드 보기
                  </Link>
                </div>
              </div>
          ...
          <div className="flex flex-col items-center justify-center py-32 bg-white rounded-[40px] border border-dashed border-stone-200 shadow-inner">
            <div className="h-24 w-24 rounded-full bg-stone-50 flex items-center justify-center mb-6">
              <Sprout className="h-12 w-12 text-stone-200 animate-pulse" />
            </div>
