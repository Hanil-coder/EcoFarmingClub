---
name: harness
description: "하네스를 구성합니다. 전문 에이전트를 정의하며, 해당 에이전트가 사용할 스킬을 생성하는 메타 스킬. Gemini CLI용. (1) '하네스 구성해줘', '하네스 구축해줘' 요청 시, (2) '하네스 설계', '하네스 엔지니어링' 요청 시, (3) 새로운 도메인/프로젝트에 대한 하네스 기반 자동화 체계를 구축할 때, (4) 하네스 구성을 재구성하거나 확장할 때, (5) '하네스 점검', '하네스 감사', '하네스 현황', '에이전트/스킬 동기화' 등 기존 하네스 운영/유지보수 요청 시 사용."
---

# Harness — Agent Team & Skill Architect (Gemini CLI)

도메인/프로젝트에 맞는 하네스를 구성하고, 각 에이전트의 역할을 정의하며, 에이전트가 사용할 스킬을 생성하는 메타 스킬.

**핵심 원칙:**
1. 에이전트 정의(`.gemini/agents/`)와 스킬(`.gemini/skills/`)을 생성한다.
2. **서브 에이전트를 기본 실행 모드로 사용한다.** Gemini CLI는 `Agent` 도구로 서브 에이전트를 호출하며, `run_in_background: true`로 병렬 실행한다.
3. **GEMINI.md에 하네스 포인터를 등록한다.** — 새 세션에서 오케스트레이터 스킬이 트리거되도록 최소한의 포인터(트리거 규칙 + 변경 이력)만 기록한다.
4. **하네스는 고정물이 아니라 진화하는 시스템이다.** — 매 실행 후 피드백을 반영하고, 에이전트·스킬·GEMINI.md를 지속 갱신한다.

> **참고 (Claude Code와의 차이):** Gemini CLI에는 `TeamCreate`, `SendMessage`, `TaskCreate` 같은 에이전트 팀 API가 없다. 모든 멀티 에이전트 조율은 `Agent` 도구(서브 에이전트) + 파일 기반 데이터 전달로 구현한다.
...
**서브 에이전트가 기본 모드다.** Gemini CLI에서는 `Agent` 도구로 서브 에이전트를 호출한다. 독립적 작업은 `run_in_background: true`로 병렬 실행하고, 결과는 오케스트레이터가 파일 또는 반환값으로 수집한다.

| 모드 | 언제 사용 | 특성 |
|------|----------|------|
| **서브 에이전트** (기본) | 단일 또는 복수 에이전트가 독립적으로 작업하고 결과를 오케스트레이터에 반환 | `Agent` 도구 호출, `run_in_background: true`로 병렬 실행 가능 |
| **하이브리드** | Phase마다 특성이 다를 때 — 예: 병렬 수집 → 순차 통합 | Phase 단위로 병렬/순차를 구분하여 구성 |

**의사결정:**
1. 에이전트들이 독립적으로 작업하고 오케스트레이터에게만 결과를 반환하면 → **서브 에이전트**
2. Phase별 특성이 확연히 다르면(예: Phase 1 병렬 수집 → Phase 2 순차 통합) → **하이브리드**

> 상세 패턴과 에이전트 분리 기준은 `references/agent-design-patterns.md` 참조.

#### 2-2. 아키텍처 패턴 선택

1. 작업을 전문 영역으로 분해
2. 에이전트 구조 결정 (아키텍처 패턴은 `references/agent-design-patterns.md` 참조)
   - **파이프라인**: 순차 의존 작업
   - **팬아웃/팬인**: 병렬 독립 작업
   - **전문가 풀**: 상황별 선택 호출
   - **생성-검증**: 생성 후 품질 검수
   - **감독자**: 오케스트레이터가 상태 관리 및 동적 분배
...
**모든 에이전트는 반드시 `프로젝트/.gemini/agents/{name}.md` 파일로 정의한다.** 에이전트 정의 파일 없이 Agent 도구의 prompt에 역할을 직접 넣는 것은 금지한다. 이유:
- 에이전트 정의가 파일로 존재해야 다음 세션에서 재사용 가능
- 역할·원칙·프로토콜이 명시되어야 에이전트 간 협업 품질 보장
- 하네스의 핵심 가치는 에이전트(누가)와 스킬(어떻게)의 분리

**모델 설정:** 모든 에이전트는 `model: "gemini-2.5-pro"`를 사용한다. Agent 도구 호출 시 반드시 `model: "gemini-2.5-pro"` 파라미터를 명시한다. 하네스의 품질은 에이전트의 추론 능력에 직결된다.

각 에이전트를 `프로젝트/.gemini/agents/{name}.md`에 정의한다. 필수 섹션: 핵심 역할, 작업 원칙, 입력/출력 프로토콜, 에러 핸들링, 협업.

> 정의 템플릿과 실제 파일 전문은 `references/agent-design-patterns.md`의 "에이전트 정의 구조" + `references/team-examples.md` 참조.

**QA 에이전트 포함 시 필수 사항:**
- QA 에이전트는 읽기/쓰기 모두 가능한 일반 에이전트 타입을 사용하라
...
각 에이전트가 사용할 스킬을 `프로젝트/.gemini/skills/{name}/SKILL.md`에 생성한다. 상세 작성 가이드는 `references/skill-writing-guide.md` 참조.
...
description은 스킬의 핵심 트리거 메커니즘이다. description을 **적극적("pushy")**으로 작성한다.
...
오케스트레이터는 스킬의 특수한 형태로, 개별 에이전트와 스킬을 하나의 워크플로우로 엮어 전체를 조율한다. Phase 4에서 생성한 개별 스킬이 "각 에이전트가 무엇을 어떻게 하는가"를 정의한다면, 오케스트레이터는 "누가 언제 어떤 순서로 협업하는가"를 정의한다. 구체적 템플릿은 `references/orchestrator-template.md` 참조.
...
**서브 에이전트 패턴 (기본):**
오케스트레이터가 `Agent` 도구로 서브 에이전트를 직접 호출한다. 병렬 실행은 `run_in_background: true`, 결과는 오케스트레이터에게만 반환된다.

```
[오케스트레이터]
    ├── Agent(agent-1, run_in_background=true)
    ├── Agent(agent-2, run_in_background=true)
    ├── 결과 대기 및 수집 (파일 Read 또는 반환값)
    └── 통합 산출물 생성
```

**하이브리드 패턴:**
Phase마다 다른 패턴을 섞어 구성한다. 자주 쓰이는 조합:
- **병렬 수집 → 순차 통합**: Phase 1에서 서브 에이전트로 독립 자료를 병렬 수집 → Phase 2에서 오케스트레이터가 직접 통합
- **순차 생성 → 병렬 검증**: Phase 1에서 단일 에이전트가 초안 생성 → Phase 2에서 복수 에이전트가 병렬로 독립 검증

하이브리드 선택 시 오케스트레이터의 각 Phase 섹션 상단에 해당 Phase의 실행 모드를 명시한다 (예: `**실행 모드:** 병렬 서브 에이전트`).
...
| **반환값 기반** | `Agent` 도구의 반환 메시지 | 서브 에이전트 결과를 오케스트레이터가 직접 수집 |

**권장 조합:** 파일 기반(산출물 저장) + 반환값 기반(요약 수집)
...
> 에이전트가 많을수록 조율 오버헤드가 커진다. 3명의 집중된 에이전트가 5명의 산만한 에이전트보다 낫다.
...
**트리거:** {도메인} 관련 작업 요청 시 `.gemini/skills/{orchestrator-skill-name}/SKILL.md`를 읽고 워크플로우를 따른다. 단순 질문은 직접 응답 가능.
...
**GEMINI.md에 넣지 않는 것:** 에이전트 목록, 스킬 목록, 디렉토리 구조, 실행 규칙 상세. 이유: 에이전트/스킬 목록은 오케스트레이터 스킬과 `.gemini/agents/`, `.gemini/skills/`에서 관리하므로 중복이다. GEMINI.md는 **포인터(트리거 규칙) + 변경 이력**만 담는다.
...
- 모든 에이전트 파일이 올바른 위치(`.gemini/agents/`)에 있는지 확인
...
2. **With-skill vs Without-skill 비교 실행** — 스킬 있는 실행과 없는 실행을 병렬로 수행하여 스킬의 부가가치를 확인한다:
   - **With-skill**: 스킬을 읽고 작업 수행
   - **Without-skill (baseline)**: 같은 프롬프트를 스킬 없이 수행

3. **결과 평가** — 정성적(사용자 리뷰) + 정량적(assertion 기반) 평가를 수행한다.

4. **반복 개선 루프** — 피드백을 **일반화**하여 스킬을 수정하고 재테스트한다.

5. **반복 패턴 번들링** — 공통으로 작성하는 코드가 발견되면 `scripts/`에 미리 번들링한다.
...
모든 변경은 GEMINI.md의 **변경 이력** 테이블에 기록한다:
...
- 대규모 변경 시 Phase 6-3(실행 테스트), 6-5(드라이런)까지 수행

## 산출물 체크리스트

생성 완료 후 확인:

- [ ] `프로젝트/.gemini/agents/` — **에이전트 정의 파일 필수 생성**
- [ ] `프로젝트/.gemini/skills/` — 스킬 파일들 (SKILL.md + references/)
- [ ] 오케스트레이터 스킬 1개 (데이터 흐름 + 에러 핸들링 + 테스트 시나리오 포함)
- [ ] 실행 모드 명시 (서브 에이전트 / 하이브리드 중 선택)
- [ ] 모든 Agent 호출에 `model: "gemini-2.5-pro"` 파라미터 명시
- [ ] 기존 에이전트/스킬과 충돌 없음
...
- [ ] **GEMINI.md에 하네스 포인터 등록** (트리거 규칙 + 변경 이력)
- [ ] **GEMINI.md 변경 이력에 에이전트/스킬 추가/삭제/수정 기록**
