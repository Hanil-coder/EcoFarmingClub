...
// FORCE DYNAMIC RENDER to bypass any persistent CDN/Build cache
...
export default async function CropDetailPage({ params }: { params: { id: string } }) {
  ...
  // DEFINITIVE FALLBACK LOGIC
  // This will show if the DB value is missing or hasn't synced
  const getFallbackTip = (name: string, currentTip: string | null) => {
    const trimmedName = name.trim();
    if (currentTip && currentTip.length > 20) return currentTip;
    ...
    return fallbackTips[trimmedName] || '이 작물을 위한 특별한 팁이 아직 등록되지 않았습니다. 커뮤니티에 질문을 남겨보세요!';
  };
  ...
  return (
    <div className="bg-brand-beige min-h-screen pb-20">
      <div className="mx-auto max-w-4xl px-4 pt-10">
        {/* DEBUG MARKER - TO CONFIRM VERSION */}
        <div className="mb-4 text-[10px] text-stone-300">Last Sync: 2026-05-22 13:50 (Dynamic)</div>
          ...
          <div className="p-8">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-stone-400 italic text-sm">{crop.scientific_name || 'Urban Crops'}</p>
              </div>
            ...
            <p className="text-stone-600 leading-relaxed mb-10 whitespace-pre-wrap text-lg">
              {crop.description}
            </p>
            ...
            <div className="border-t border-stone-100 pt-8">
              ...
              <div className="bg-brand-green-light/20 p-8 rounded-[32px] text-stone-700 text-base leading-relaxed border border-brand-green/10 shadow-inner">
                {cultivationTip}
              </div>
            </div>
          </div>
